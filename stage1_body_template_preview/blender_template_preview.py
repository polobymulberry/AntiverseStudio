...
def check_prerequisites_for_all_templates() -> int:
    """导入每个模板的模型，确认存在名为 body 的根物体后清理；不渲染。"""
    template_dirs = sorted(p for p in SETTINGS.template_root.iterdir() if p.is_dir())
    failures: list[str] = []
    skipped: list[str] = []
    ...
    if failures:
        ...
        return 1
    print(f"前提检查通过（已检查 {len(template_dirs) - len(skipped)} 个模板）")
    return 0
...
def main() -> None:
    ...
    if args.check_prerequisite:
        raise SystemExit(check_prerequisites_for_all_templates())
